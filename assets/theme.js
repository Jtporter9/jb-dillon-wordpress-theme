...
theme.Product = (function () {
  ...
  Product.prototype = _.assignIn({}, Product.prototype, {
    ...
    _updateCartPopupContent: function (item) {
      ...
      $.getJSON('/cart.js').then(
        function (cart) {
          ...
          this._showCartPopup();
          document.getElementById("opaqueBackground").classList.remove("hide");
        }.bind(this)
      );
    },
    ...
    _hideCartPopup: function (event) {
      ...
      $('body').off('click');
      document.getElementById("opaqueBackground").classList.add('hide');
    },
...
$(document).ready(function () {
  $.getJSON('/cart.js').then(
    function (cart) {
      ...
      $('*[data-cart-popup-cart-quantity]').text(quantity).attr('aria-label', ariaLabel);

      // SET CART COUNT BUBBLE
      $('*[data-cart-count-bubble]').removeClass('hide');
      $('*[data-cart-count]').text(quantity);
    }.bind(this)
  );
});

// ON PRODUCT SUBMIT 
$('.product-form product-form-product-template').submit(function() {
  console.log("foobars")
})
...
document.getElementById("floatingCart").onclick = function () {
  ...
  $.getJSON('/cart.js').then(
    function (cart) {
      ...
      $('*[data-cart-count-bubble]').removeClass('hide');
      $('*[data-cart-count]').text(quantity);
      // SET DATA CART POPUP IMAGE WRAPPER
  ...
  if (isLoading) {
    ...
    $('.shopify-payment-button').attr('disabled', true);
    ...
    $('.shopify-payment-button').removeAttr('disabled');
    $('*[data-loader-status]').attr('aria-hidden', true);
  }
  document.getElementById("opaqueBackground").classList.remove("hide");
};

document.getElementById("closePopupCustom").onclick = function (event) {
  // HIDE CART POPUP
  var setFocus = event.detail === 0 ? true : false;
  $('*[data-cart-popup-wrapper]')
    .prepareTransition()
...
document.getElementById("opaqueBackground").onclick = function () {
  // HIDE CART POPUP
  var setFocus = event.detail === 0 ? true : false;
  $('*[data-cart-popup-wrapper]')
    .prepareTransition()
